...
class PandocCaptionWidget extends WidgetType {
	...
	toDOM(view: EditorView): HTMLElement {
		...
		caption.className = `captions-pandoc-editor-caption captions-pandoc-editor-caption--${this.target.kind}`;
		...
		if (this.labelText !== null) {
			...
			if (this.target.caption.length > 0) {
				caption.append(": ", this.target.caption);
			}
		} else {
			caption.textContent = this.target.caption;
		}
		return caption;
	}
}
