...
import { createQuartoEditorExtension } from "../src/features/quarto/live-preview";
import type { QuartoSettings } from "../src/features/quarto/settings";
import {
...
const CAPTION_SETTINGS: QuartoSettings = {
	figureLabel: "Figure",
	tableLabel: "Table",
	alignment: "center",
	style: "bold",
...
void test("provides Quarto block decorations from editor state", () => {
	const { document } = parseHTML("<main></main>");
	const livePreviewField = createLivePreviewField();
	const extension = createQuartoEditorExtension(
		() => CAPTION_SETTINGS,
		livePreviewField,
	...
	assert.deepEqual(
		...
		[
			["captions-quarto-editor-caption captions-quarto-editor-caption--figure captions-caption captions-caption--center captions-caption--bold captions-caption--figure-below", true, 1],
			["captions-quarto-editor-caption captions-quarto-editor-caption--table captions-caption captions-caption--center captions-caption--bold captions-caption--table-above", true, -1],
		],
	);
	assert.equal(
		decorations.filter((item) => item.className === "captions-quarto-editor-reference").length,
		2,
	);
	assert.equal(
...
void test("refreshes Quarto decorations for selections, edits, settings, and mode", () => {
	const { document } = parseHTML("<main></main>");
	...
	const referencePosition = SOURCE.lastIndexOf("@fig-architecture") + 2;
	state = state.update({ selection: { anchor: referencePosition } }).state;
	assert.equal(
		renderDecorations(state, extension, document)
			.filter((item) => item.className === "captions-quarto-editor-reference").length,
		1,
	);
