...
设置页面提供共享的对齐、字体样式、相对字号、上下间距、图片/表格题注位置和可选文件名兜底。字号和题注上下间距均使用数字输入框，并以占位符显示默认值 85% 和 12px；清空输入框会使用对应默认值，题注间距用于分隔题注与对应图片或表格，不替代主题控制的外围段落间距。设置页面跟随 Obsidian 的英文或简体中文界面。

题注在本地渲染，不会修改笔记源文档。阅读模式和实时预览使用相同的题注规则与外观。带题注的图片和表格在阅读模式中保持与其他内容块一致的段落间距。Source mode、普通链接、行内 Markdown 图片、未匹配的表格和交叉引用样式的文本都会保持原样。
