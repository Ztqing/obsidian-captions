...
设置界面会跟随 Obsidian 当前语言，目前补齐英文和简体中文。数值外观项会在滑块旁常驻显示当前值，并提供可用键盘编辑的数字输入框；字号支持 50% 至 200% 范围内按 1% 精确输入，间距支持 0px 至 32px 范围内按 1px 输入。Pandoc 与 Quarto 共用图片和表格标签；对齐、字体样式、相对字号、上下间距以及图表题注位置同时作用于阅读模式与实时预览中由本插件生成的全部题注。字号默认 85%；上下间距默认均为 12px。字体样式默认为粗体。图片题注默认位于图片下方，表格题注默认位于表格上方。文件名兜底继续作为共享题注行为。已有 `0.0.2` 至 `0.0.7` 设置会经过校验后迁移，并保持原有持久化 key 不变。

`0.0.8` 改进了设置界面的本地化和数值输入体验。
...
`0.0.8` 支持独占一行的标准 Markdown 图片、pipe table、直接裸 Quarto 引用，以及统一的题注标签、外观和行为设置。暂不支持跨笔记引用、章节编号、子图、可执行单元格标签和复杂引用组。
